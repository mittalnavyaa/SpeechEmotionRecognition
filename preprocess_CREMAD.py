...
def get_all_wav_files(dataset_path):
    """Recursively get all WAV files from the dataset directory."""
    wav_files = []
    for root, dirs, files in os.walk(dataset_path):
        for file in files:
            if file.endswith('.wav'):
                ...
                print(f"Found WAV file: {full_path}")  # Debugging print statement
    return wav_files
...
def extract_embeddings():
    ...
    dataset_path = r"C:\Users\navya\wav2vec2-lg-xlsr-en-speech-emotion-recognition\CREMA-D"
    ...
    # Get list of all WAV files from the dataset directory
    ...
    for file_path in tqdm(wav_files, desc="Processing audio files"):
        try:
            ...
            emotion = emotion_mapping.get(parts[2], None)
            if emotion is None:
    ...
    embeddings_file = os.path.join(save_path, 'embeddings_CREMAD.npy')
    labels_file = os.path.join(save_path, 'labels_CREMAD.npy')
    ...
    np.save(os.path.join(save_path, 'embeddings_CREMAD.npy'), embeddings)
    np.save(os.path.join(save_path, 'labels_CREMAD.npy'), labels)
...
for file in os.listdir(save_path):
    if file.startswith('embeddings_CREMAD') or file.startswith('labels_CREMAD'):
        print(f"- {file}")
